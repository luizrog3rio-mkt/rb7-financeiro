-- APLICADA: 20260630204455
-- Pedido do Luiz 2026-06-30: a dre_by_competency passa a rotear o bruto Hotmart pela conta DIRETA
-- (hotmart_product_map.chart_of_account_id) com prioridade, caindo no Produto DRE -> conta
-- (dre_products.chart_of_account_id) se nao houver direta, senao "a classificar". Unica mudanca vs
-- a versao anterior: dp.chart_of_account_id -> coalesce(pm.chart_of_account_id, dp.chart_of_account_id)
-- no ramo Hotmart (mov + hot). Total preservado. Aprovado pelo Luiz em 2026-06-30. Verificado:
-- mapear "Usando a Oratoria pra Viralizar" direto -> conta 1.2.01 moveu R$1.521 de HOT-1 -> a conta
-- na DRE de 2025 (onde estao as vendas dele), exato.
